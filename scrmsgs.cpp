...
sScrMsg::~sScrMsg()
{
	if (data.type == kMT_String || data.type == kMT_Vector)
		g_pMalloc->Free(data.psz);
	if (data2.type == kMT_String || data2.type == kMT_Vector)
		g_pMalloc->Free(data.psz);
	if (data3.type == kMT_String || data3.type == kMT_Vector)
		g_pMalloc->Free(data.psz);
}
